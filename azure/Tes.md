...
If you want to know more about the different types of testing, we recommend you reading this article: Test ASP.NET Core MVC apps.
...
Click on New Pipeline button (or Create Pipeline if you haven't any pipeline).
...
Click the Save button (not Save and run) on the top-right part of the page to save the pipeline definition. You can find the Save button by clicking on the arrow to the right of the Save and Run (or Run) button.
...
Edit the pipeline you created in the previous task pressing the Edit button.

Now you need to add the Integration Tests task after the Unit Tests task:
...
Then you need to add the Functional tests task after the Integration Tests task:
...
Click Validate and Save button, and if validation is successful, click Save again to commit the changes directly into the main branch.

Task 3: Check the tests summary
Click on the Run, then from the Run pipeline tab, click on Run again.

Wait for the pipeline to start and until it completes the Build Stage successfully.

Once completed, the Test tab will show as part of the pipeline run. Click on it to check the summary. It looks like shown below:

Screenshot of the tests summary.

For more details, at the bottom of the page, the table shows a list of the different run tests.

Note: If the table is empty, you need to reset the filters to have all the details about the tests run.

Screenshot of the tests table.
