...
Click on Create Pipeline or New Pipeline button
...
Select the main branch and the /.ado/eshoponweb-ci-pr.yml file, then click on Continue
...
Your pipeline will take a name based on the project name. Let's rename it for identifying the pipeline better. Go to Pipelines > Pipelines and click on the recently created pipeline. Click on the ellipsis and Rename/Move option. Name it eshoponweb-ci-pr and click on Save.
...
Navigate to the Repos section in the eShopOnWeb navigation and click Branches.

Create a new branch named Feature01 based on the main branch.
...
Make the following change on the first line:

// Testing my PR
Click on Commit > Commit (leave default commit message).

A message will pop-up, proposing to create a Pull Request (as your Feature01 branch is now ahead in changes, compared to main). Click on Create a Pull Request.
...
After all validations are successful, on the top-right click on Approve. Now from the Set auto-complete dropdown you can click on Complete.

On the Complete Pull Request tab, click on Complete Merge
...
Click on Run and wait for the pipeline to execute successfully.

Task 2: Enable Continuous Integration
The default build pipeline definition doesn't enable Continuous Integration.

Click on the Edit pipeline option under the ellipsis menu near Run new button in the top-right

Now, you need to replace the # trigger: and # - main lines with the following code:
...
Click the Validate and save button to validate and save the pipeline definition.

Select Create a new branch for this commit.

Keep the default branch name and Start a pull request checked.
...
Click on the "Update eshoponweb-ci.yml for Azure Pipelines" pull request.

After all validations are successful, on the top-right click on Approve. Now you can click on Complete.
...
Navigate to the Repos section, and click on Branches.

Create a new branch named Feature02 based on the main branch.

Click the new Feature02 branch.

Navigate to the /eShopOnWeb/src/Web/Program.cs file and click Edit in the top-right.
...
A message will pop-up, proposing to create a Pull Request (as your Feature02 branch is now ahead in changes, compared to main).
...
Go back to Pipelines > Pipelines, you will notice that the build eshoponweb-ci was triggered automatically after the code was merged.

Click on the eshoponweb-ci build then select the last run.

After its successful execution, click on Related > Published to check the published artifacts:
