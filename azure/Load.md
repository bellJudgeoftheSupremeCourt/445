...
From the lab computer, start a web browser, navigate to the Azure Portal, and sign in.
...
Navigate back to the Pipelines pane in of the Pipelines hub.

Click New pipeline (or Create Pipeline if this is the first one you create).

Note: We will use the wizard to create a new YAML Pipeline definition based on our project.

On the Where is your code? pane, click Azure Repos Git (YAML) option.

On the Select a repository pane, click eShopOnWeb.

On the Configure your pipeline pane, scroll down and select Starter Pipeline.

Select all lines from the Starter Pipeline, and delete them.

Copy the full template pipeline from below, knowing you will need to make parameter modifications before saving the changes:
...
Set the cursor on a new line at the end of the YAML definition. Make sure you position the cursor at the indentation of the previous task level.

Note: This will be the location where new tasks are added.

Click Show Assistant from the right hand side of the portal. In the list of tasks, search for and select the Azure App Service Deploy task.

In the Azure App Service deploy pane, specify the following settings and click Add:
...
Confirm the settings from the Assistant pane by clicking the Add button.

Note: This will automatically add the deployment task to the YAML pipeline definition.

The snippet of code added to the editor should look similar to below, reflecting your name for the azureSubscription and WebappName parameters:
...
Before saving the updates to the yml-file, give it a more clear name. On top of the yaml-editor window, it shows EShopOnweb/azure-pipelines-#.yml. (where # is a number, typically 1 but could be different in your setup.) Select that filename, and rename it to m08l14-pipeline.yml

Click Save, on the Save pane, click Save again to commit the change directly into the main branch.
...
From the Azure DevOps left menu, navigate to Pipelines and select Pipelines again. Next, select All to open all pipeline definitions, not just the Recent ones.
...
Screenshot of Azure Pipelines showing eShopOnWeb runs.

Confirm to run this pipeline by clicking Run pipeline from the appearing pane and confirm by clicking Run once more.

Notice the 2 different Stages, Build .Net Core Solution and Deploy to Azure Web App appearing.

Wait for the pipeline to kick off.

Ignore any Warnings showing up during the Build Stage. Wait until it completes the Build Stage successfully. (You can select the actual Build stage to see more details from the logs.)

Once the Deploy Stage wants to start, you are prompted with Permissions Needed, as well as an orange bar saying:

This pipeline needs permission to access a resource before this run can continue to Deploy to an Azure Web App
Click on View

From the Waiting for Review pane, click Permit.

Validate the message in the Permit popup window, and confirm by clicking Permit.

This sets off the Deploy Stage. Wait for this to complete successfully.
...
Important: Azure Load Testing is a paid service. You will incur costs for running load tests. Make sure to clean up resources after you have completed the lab to avoid incurring additional costs. For each 'Load Testing Resource' that is active during any part of a month you will be charged the monthly fee, and have access to the included 50 VUH. See the Azure Load Testing pricing page for more information.

Task 1: Deploy Azure Load Testing
In this task, you will deploy an Azure Load Testing Resource into your Azure subscription.

From the Azure Portal (https://portal.azure.com), navigate to Create Azure Resource.

In the 'Search Services and marketplace' search field, enter Azure Load Testing.

Select Azure Load Testing (published by Microsoft), from the search results.

From the Azure Load Testing Page, click Create to start the deployment process.

From the 'Create a Load Testing Resource' page, provide the necessary details for the resource deployment:
...
Click Review and Create, to have your settings validated.

Click Create to confirm, and get the Azure Load Testing resource deployed.

You are switched to the 'Deployment is in progress' page. Wait for a few minutes, until the deployment completes successfully.

Click Go to Resource from the deployment progress page, to navigate to the eShopOnWebLoadTesting Azure Load Testing resource.
...
From within the eShopOnWebLoadTesting Azure Load Testing Resource blade, navigate to Tests under Tests. Click the + Create menu option, and select Create a URL-based test.

Uncheck the Enable advanced settings checkbox, to display the advanced settings.

Complete the following parameters and settings to create a load test:
...
Confirm the configuration of the test, by clicking Review and Create, (Don't make any changes in the other tabs). Click Create once more.

This kicks off the Load Testing tests, which will run The test will run for 5 minutes.

With the test running, navigate back to the eShopOnWebLoadTesting Azure Load Testing Resource page, and navigate to Tests, select Tests and see a test Get_eshoponweb...

From the top menu, click Create, Create a URL-based test, to create a 2nd Load test.

Complete the following parameters and settings to create another load test:
...
Test Duration (minutes): 5
Ramp-up time (minutes): 1
Confirm the configuration of the test, by clicking Review + create, and Create once more.

The test will run for about 5 minutes.
...
From Azure Load Testing, navigate to Tests. Select either of the test definitions, to open a more detailed view, by clicking on one of the tests. This redirects you to the more detailed test page. From here, you can validate the details of the actual runs, by selecting the TestRun_mm/dd/yy-hh:hh from the resulting list.

From the detailed TestRun page, identify the actual outcome of the Azure Load Testing simulation. Some of the values are:
...
More below, several of these values are represented using dashboard graph line and chart views.

Take a few minutes to compare the results of both simulated tests with each other, and identify the impact of more users on the App Service performance.
...
From the Azure DevOps Portal(https://aex.dev.azure.com), navigate to the eShopOnWeb Project.
From the down left corner, select Project Settings.
Under the Pipelines section, select Service Connections.
Notice the Service Connection, having the name of your Azure Subscription you used to deploy Azure Resources at the start of the lab exercise.
Select the Service Connection. From the Overview tab, navigate to Details and select Manage service connection roles.
This redirects you to the Azure Portal, from where it opens the resource group details in the access control (IAM) blade.
...
In the Members tab, select Select members, and then find and select your user account and click Select.
...
Use any zip tool to extract the input files. The folder contains the following files:
...
Commit all extracted input files to your source control repository. To do this, navigate to the Azure DevOps Portal(https://aex.dev.azure.com/), and navigate to the eShopOnWeb DevOps Project.

Select Repos. In the source code folder structure, notice the tests subfolder. Notice the ellipsis (...), and select New > Folder.

specify jmeter as folder name, and placeholder.txt for the file name (Note: a Folder cannot be created as empty)

Click Commit to confirm the creation of the placeholder file and jmeter folder.

From the Folder structure, navigate to the new created jmeter subfolder. Click the ellipsis(...) and select Upload File(s).

Using the Browse option, navigate to the location of the extracted zip-file, and select both config.yaml and quick_test.jmx.

Click Commit to confirm the file upload into source control.

Within Repos, browse to the /tests/jmeter subfolder just created.

Open the Load Testing config.yaml file. Click Edit to allow editing of the file.

Replace the displayName and testId attributes with the value ado_load_test

Screenshot of the edited config file.

Task 4: Update the CI/CD workflow YAML definition file
To create and run a load test, the Azure Pipelines workflow definition uses the Azure Load Testing task extension from the Azure DevOps Marketplace. Open the Azure Load Testing task extension in the Azure DevOps Marketplace, and select Get it free.

Select your Azure DevOps organization, and then select Install to install the extension.

From within the Azure DevOps Portal and Project, navigate to Pipelines and select the pipeline created at the start of this exercise. Click Edit.

In the YAML script, navigate to line 64 and press ENTER/RETURN, to add a new empty line. (this is right before the Deploy Stage of the YAML file).

At line 65, select the Tasks Assistant to the right-hand side, and search for Azure Load Testing (make sure you position the cursor at the indentation of the previous task level).

Complete the graphical pane with the correct settings of your scenario:
...
Confirm the injection of the parameters as a snippet of YAML by clicking Add

If the indentation of the YAML snippet is giving errors (red squiggly lines), fix them by adding 2 spaces or tab to position the snippet correctly.

The below sample snippet shows what the YAML code should look like
...
below the inserted YAML snippet, add a new empty line by pressing ENTER/RETURN.

below this empty line, add a snippet for the Publish task, showing the results of the Azure Load testing task during the pipeline run:

- publish: $(System.DefaultWorkingDirectory)/loadTest
  artifact: loadTestResults
If the indentation of the YAML snippet is giving errors (red squiggly lines), fix them by adding 2 spaces or tab to position the snippet correctly.

With both snippets added to the CI/CD pipeline, click on Validate and save and, then, Save the changes.

Once saved, click Run to trigger the pipeline.

Confirm the branch (main) and click the Run button to start the pipeline run.

From the pipeline status page, click the Deploy stage to open the verbose logging details of the different tasks in the pipeline.

Wait for the pipeline to kick off the Deploy Stage, and arrive at the AzureLoadTest task in the flow of the pipeline.

While the task is running, browse to the Azure Load Testing in the Azure Portal, and see how the pipeline creates a new RunTest, named ado_load_test. You can select it to show the outcome values of the TestRun job.

Navigate back to the Azure DevOps CI/CD Pipeline Run view, where the AzureLoadTest task completed successfully. From the verbose logging output, the resulting values of the load test will be visible as well:
...
You have now performed an automated Load Test as part of a pipeline run. In the last task, you will specify conditions for failure, meaning, we will not allow our deploy Stage to start, if the performance of the web app is below a certain threshold.
...
From Azure DevOps, navigate to the eShopOnWeb Project, and open Repos.

Within Repos, browse to the /tests/jmeter subfolder created and used earlier.

Open the Load Testing *config.yaml** file. Click **Edit** to allow editing of the file.

Replace failureCriteria: [] if present, otherwise append the following snippet of code:

failureCriteria:
  - avg(response_time_ms) > 300
  - percentage(error) > 50
Save the changes to the config.yaml by clicking Commit and Commit once more.

Navigate back to Pipelines and run the eShopOnWeb pipeline again. After a few minutes, it will complete the run with a failed status for the AzureLoadTest task.

Open the verbose logging view for the pipeline, and validate the details of the AzureLoadtest. A similar sample output is below:
...
Notice how the last line of the Load testing output says ##[error]TestResult: FAILED; since we defined a FailCriteria having an avg response time of > 300, or having an error percentage of > 20, now seeing an avg response time which is more than 300, the task will be flagged as failed.
...
The FAILED status of the pipeline task, actually reflects a SUCCESS of the Azure Load Testing requirement criteria validation.
